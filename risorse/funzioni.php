...
$ricercaProdotti = query('SELECT * FROM ricetta ');
...
while ($row = fetch_array($ricercaProdotti)){
    ...
    <div class="card h-100">
      ...
      <div class="card-body">
        <h4 class="card-title">
          <a href="prodotto.php?id={$row['id_ricetta']}">{$row['titolo']}</a>
        </h4>
        <h5>Minuti preparazione: {$row['tempo_preparazione']}</h5>
        
        <p class="card-text">Note: {$row['note']}</p>
      </div>
      <div class="card-footer text-center">
      <h5>Livello difficoltà: {$row['difficoltà']}</h5>
     <a href="prodotto.php?id={$row['id_ricetta']}" class="btn btn-success btn-small">Dettagli</a>
      </div>
    </div>
  </div>
STRINGA_PDT;
...
function mostraCategorie(){
  ...
  $ricercaCategorie = query('SELECT * FROM tipologia_portata');
...
<a href='categorie.php?id={$row['id_tipologia_portata']}' class='list-group-item'>{$row['nome']}</a>
...
$pdtCategoria = query("SELECT * FROM ricetta WHERE tipologia_portata_id_tipologia_portata = {$_GET['id']}");
...
function ricerca(){
  ...
  $ricerca = query("SELECT * FROM ricetta WHERE titolo  LIKE  '%$ricercaUtente%' ");
  echo $ricerca;
  conferma($ricerca);
  }
  ...
  <div class="card-body">
    ...
    <h4 class="mb-5">€{$row['difficoltà']}</h4>
    <h5>Info</h5>
